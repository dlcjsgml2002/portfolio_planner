...
<script>
	...
	function get_plan_info(pno) {
		$.ajax({
			url : "${pageContext.request.contextPath}/calendar/modal/insert",
			type : "get",
			data : {"pno": pno},
			success : function(json) {
				console.log(json)
...
<section>
	...
	<thead>
	  ...
	  <th style="text-align: center; font-size: 25px"><fmt:formatDate value="${map.today }" pattern="yyyy-MM-dd"/></th>
		<th><button class="btn btn-primary" onclick="add_plan()">플랜 추가</button></th>
	  </tr>
	</thead>
	<tbody>
	 <c:forEach var="plan" items="${map.plan }">
	   <tr>
	<td>
		<a href="javascript:void(0)" onclick="get_plan_info(${plan.pno })">
		<div class="plan_item_div"> ${plan.title }</div>
		</a>
	</td>
